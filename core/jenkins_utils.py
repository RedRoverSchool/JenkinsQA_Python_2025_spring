...
def get_page(session, url, config):
    ...
    if not response.ok:
        ...
        session.post(
            config.jenkins.login_url, headers=headers, data=config.jenkins.login_data
        )
        response = session.get(url, headers=headers)
    if not response.ok:
...
def delete_jobs_views(session, config):
    ...
    view_page = get_page(
        session, config.jenkins.base_url + "/me/my-views/view/all/", config
    )
    crumb = get_crumb(main_page)

    url = (
        config.jenkins.base_url
        + f"/user/{config.jenkins.USERNAME}"
        + "/my-views/view/{}/doDelete"
    )
    names = get_substrings(
        view_page,
        f'href="/user/{config.jenkins.USERNAME}/my-views/view/',
        '(?:/"|" class)',
    ) - {"all/newJob", "all/builds"}
...
def delete_users(session, config):
    user_page = get_page(
        session, config.jenkins.base_url + "/manage/securityRealm/", config
    )
...
def delete_domains(session, config):
    system_page = get_page(
        session, config.jenkins.base_url + "/manage/credentials/store/system/", config
    )
    url = (
        config.jenkins.base_url + "/manage/credentials/store/system/domain/{}/doDelete"
    )
