...
def wait_for(driver, by, selector, timeout=DEFAULT_TIMEOUT):
    return wait(driver, timeout).until(EC.visibility_of_element_located((by, selector)))


def wait_for_clickable(driver, by, selector, timeout=DEFAULT_TIMEOUT):
    return wait(driver, timeout).until(EC.element_to_be_clickable((by, selector)))
...
def test_check_create_new_item(new_item_page, main_page):
    ...
    folder_option = wait_for_clickable(
        new_item_page, By.CSS_SELECTOR, '[class*="cloudbees_hudson_plugins_folder"]'
    )
    folder_option.click()
