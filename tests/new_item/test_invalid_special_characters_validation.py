...
def click_new_item_button(driver):
    try:
        new_item_button = driver.find_element(
            By.XPATH, "//a[contains(@href, '/newJob')]"
        )
...
class TestNewItemSpecialCharacters:
    def test_special_characters_not_allowed(self, login_page, main_page, driver):
        assert login_page.title == "Sign in [Jenkins]" or main_page.title in [
            "Dashboard [Jenkins]",
            "ИнфоПанель [Jenkins]",
        ]
        ...
        assert "unsafe character" in text, (
            f"Expected 'unsafe character' in the error message, but got: '{text}'"
        )
