...
def test_save_on_folder_configurator(main_page):
    ...
    wait.until(
        EC.visibility_of_element_located((By.NAME, "_.displayNameOrNull"))
    ).send_keys(test_folder)

    wait.until(EC.presence_of_element_located((By.NAME, "Submit"))).click()

    h1_element = wait.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, "#main-panel > h1"))
    ).text.strip()
    wait.until(EC.presence_of_element_located((By.ID, "main-panel")))
    assert h1_element == test_folder, (
        f"Expected name text to be '{test_folder}', but got '{h1_element}'"
    )
