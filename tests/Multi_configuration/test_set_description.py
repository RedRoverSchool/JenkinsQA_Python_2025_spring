...
def test_set_description_for_multi_config_project(multi_config_project_page):
    ...
    wait.until(EC.presence_of_element_located((By.NAME, "description"))).send_keys(
        description_text
    )
    ...
    assert description_text in desc_element.text, (
        f"Expected '{description_text}', but got '{desc_element.text}'"
    )
