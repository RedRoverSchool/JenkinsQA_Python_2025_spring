...
def test_user_can_trigger_builds_remotely(
    revoke_project_tokens, auth_token, freestyle, config
):
    ...
    checkbox = freestyle.find_element(
        By.CSS_SELECTOR, "input[name='pseudoRemoteTrigger']~label"
    )
    ...
    wait10.until(EC.visibility_of_element_located((By.NAME, "authToken"))).send_keys(
        token
    )
    ...
    wait10.until(
        EC.visibility_of_element_located((By.LINK_TEXT, "Build History"))
    ).click()
    ...
    builds = wait10.until(
        EC.visibility_of_all_elements_located(
            (By.CSS_SELECTOR, "#projectStatus>tbody>tr")
        )
    )

    assert len(builds) == 1, f"Expected 1 build, found {len(builds)}"
    assert freestyle.find_element(
        By.LINK_TEXT, f"{Freestyle.project_name}"
    ).is_displayed(), f"No build entry found for project '{Freestyle.project_name}'"
