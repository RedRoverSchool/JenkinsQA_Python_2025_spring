...
@pytest.mark.parametrize(
    "tp_link, tp_wait, tp_expected_text",
    [
        (
            Freestyle.tooltip_environment_link[0],
            Freestyle.tooltip_environment_link_wait[0],
            Freestyle.tooltip_environment_expected_text[0],
        ),
        (
            Freestyle.tooltip_environment_link[1],
            Freestyle.tooltip_environment_link_wait[1],
            Freestyle.tooltip_environment_expected_text[1],
        ),
    ],
)
def test_tooltip_environment_items(freestyle, tp_link, tp_wait, tp_expected_text):
    ...
    tp_text = wait.until(
        EC.visibility_of_element_located((By.XPATH, '//div[@class="tippy-content"]'))
    ).text
