...
def freestyle(main_page, config):
    ...
    wait5.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, 'a[href="/view/all/newJob"]'))
    ).click()
    wait5.until(EC.presence_of_element_located((By.ID, "name"))).send_keys(
        Freestyle.project_name
    )
    main_page.find_elements(By.CSS_SELECTOR, '.j-item-options>li[tabindex="0"]')[
        0
    ].click()
...
def tooltip(freestyle):
    ...
    tooltip_link = freestyle.find_element(
        By.XPATH, '//span[@tooltip="Enable or disable the current project"]'
    )
    actions.move_to_element(tooltip_link).perform()
    wait.until(
        EC.presence_of_element_located(
            (By.XPATH, '//span[@aria-describedby="tippy-15"]')
        )
    )
...
def disabled_message(freestyle):
    ...
    wait.until(
        EC.presence_of_element_located(
            (By.CLASS_NAME, "jenkins-toggle-switch__label__checked-title")
        )
    )
...
def enable_automatically(disabled_message):
    ...
    try:
        wait2.until(
            EC.presence_of_element_located((By.XPATH, '//form[@action="enable"]'))
        )
    except Exception:
        pass
    ...
    wait.until(
        EC.presence_of_element_located(
            (By.XPATH, '//label[@class="jenkins-toggle-switch__label "]')
        )
    )
    is_enable_text = disabled_message.find_element(
        By.XPATH, '//label[@class="jenkins-toggle-switch__label "]'
    ).text
...
def description_appears(freestyle):
    wait = WebDriverWait(freestyle, 10)
    freestyle.find_element(By.XPATH, '//textarea[@name="description"]').send_keys(
        Freestyle.description_text
    )
    freestyle.find_element(By.XPATH, '//button[@name="Submit"]').click()
...
def revoke_project_tokens(main_page, config):
    ...
    wait.until(
        EC.visibility_of_element_located(
            (By.CSS_SELECTOR, 'a[tooltip*="Current token(s)"]')
        )
    )

    existing_tokens = main_page.find_elements(
        By.CSS_SELECTOR, '.token-list-existing-item > input[name="tokenName"]'
    )
    ...
    if existing_tokens:
        ...
        if any(Freestyle.project_name in name for name in token_names):
            ...
            revoke_links = wait.until(
                EC.visibility_of_all_elements_located(revoke_selector)
            )
            ...
            for link in revoke_links:
                ...
                wait.until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, "button[data-id='ok']")
                    )
                ).click()
...
def auth_token(main_page, config):
    ...
    wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "button.repeatable-add"))
    ).click()
    token_name_input = wait.until(
        EC.visibility_of_element_located(
            (By.CSS_SELECTOR, "input[placeholder='Default name']")
        )
    )
    token_name_input.send_keys(Freestyle.project_name)
    wait.until(
        EC.element_to_be_clickable((By.ID, "api-token-property-token-save"))
    ).click()
    token_element = wait.until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "button[title='Copy this token'"))
    )
