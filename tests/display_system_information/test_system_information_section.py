...
from tests.display_system_information.system_information_page import (
    SystemInformationPage as SIP,
)
...
class TestSystemInformationSection:
    ...
    def test_system_properties_tab_show_hide_all_values(self, sys_info_page):
        ...
        for element_name in element_names:
            assert page.is_value_displayed(element_name), (
                f"Value for {element_name} is still hidden"
            )

        page.click(SI.HIDE_SYS_VALUES_BUTTON)
        for element_name in element_names:
            button_locator = page.define_show_single_value_button_locator(element_name)
            assert page.is_clickable(button_locator), (
                f"Button for {element_name} is not clickable"
            )
    ...
    def test_system_properties_tab_show_hide_each_single_value(self, sys_info_page):
        ...
        for element_name in element_names:
            ...
            assert page.is_clickable(show_button), (
                f"Button for {element_name} is not clickable"
            )
    ...
    def test_plugins_tab_display_plugins_info(self, sys_info_page):
        ...
        for i in range(1, number_of_plugins + 1):
            name_locator, version_locator, status_locator, plugin_name = (
                page.get_plugin_info(i)
            )
            assert page.is_visible(name_locator), (
                f"Row {i}: Plugin name is not displayed"
            )
            assert page.is_link_clickable(name_locator), (
                f"Row {i}: {plugin_name} is not clickable"
            )
            assert page.is_visible(version_locator), (
                f"Row {i}: {plugin_name} version is not displayed"
            )
            assert page.is_visible(status_locator), (
                f"Row {i}: {plugin_name} status is not displayed"
            )
            assert page.get_element_text(status_locator) in ["true", "false"], (
                f"Row {i}: {plugin_name} status is not correct"
            )
