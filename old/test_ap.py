...
def login_as_standard_user(driver):
    WAIT(driver, prod_timeout).until(
        EC.visibility_of_element_located(USER_NAME_FIELD)
    ).send_keys(STANDARD_USER_name)
    WAIT(driver, prod_timeout).until(
        EC.visibility_of_element_located(USER_PWD_FIELD)
...
def assert_error_message(driver, error_message):
    actual_error_message = (
        WAIT(driver, prod_timeout)
        .until(EC.visibility_of_element_located(REQUIRED_FIELD_ERROR_MESSAGE_TEXT))
        .text
    )
    assert actual_error_message == error_message


def assert_required_field_error_message_background_color(
    actual_error_message_color, expected
):
    try:
...
class TestSauceDemo:
    ...
    def test_standard_user_logout(self, driver):
        ...
        login_as_standard_user(driver)
        WAIT(driver, prod_timeout).until(
            EC.visibility_of_element_located(BURGER_MENU)
        ).click()
        WAIT(driver, prod_timeout).until(
    ...
    def test_username_required(self, driver):
        ...
        actual_error_message_background_color = get_css_element_color(
            driver, USER_NAME_FIELD
        )
        assert_error_message(driver, USERNAME_REQUIRED_ERROR_MSG)
        assert_required_field_error_message_background_color(
            actual_error_message_background_color,
            REQUIRED_FIELD_ERROR_MESSAGE_BACKGROUND_COLOR,
        )
        assert_message_text_font_color(
            driver, REQUIRED_FIELD_ERROR_MESSAGE_TEXT, ERROR_MESSAGE_FONT_COLOR
        )
    ...
    def test_password_masked_by_bullets(self, driver):
        ...
        password_field_type = (
            WAIT(driver, prod_timeout)
            .until(EC.visibility_of_element_located(USER_PWD_FIELD))
            .get_attribute("type")
        )
