...
INVALID_EMAIL = ["space @mail.com", "missed_atmail.com", "missed_dot@mailcom"]
...
class ElementsTextBoxPage(BasePage):
    ...
    SUBMITTED_CURRENT_ADDRESS = (
        By.XPATH,
        "//div[@id='output']//p[@id='currentAddress']",
    )
...
class TestElementsTextBoxPage:
    ...
    def test_submit_all_correct_fields(
        self, driver: webdriver.Chrome, textbox_page: ElementsTextBoxPage
    ):
        ...
        assert page.submitted_element_text(page.SUBMITTED_FULL_NAME) == TEST_USER.get(
            "full_name"
        )
        assert page.submitted_element_text(page.SUBMITTED_EMAIL) == TEST_USER.get(
            "valid_email"
        )
        assert page.submitted_element_text(
            page.SUBMITTED_CURRENT_ADDRESS
        ) == TEST_USER.get("current_address")
        assert page.submitted_element_text(
            page.SUBMITTED_PERMANENT_ADDRESS
        ) == TEST_USER.get("permanent_address")

    @pytest.mark.parametrize("invalid_email", INVALID_EMAIL)
    def test_submit_invalid_email(
        self,
        driver: webdriver.Chrome,
        textbox_page: ElementsTextBoxPage,
        invalid_email: str,
    ):
