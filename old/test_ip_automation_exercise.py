...
def test_register_user(automation_exercise, user_data):
    ...
    automation_exercise.find_element(
    ...
    automation_exercise.find_element(By.ID, "password").send_keys(
    ...
    automation_exercise.find_element(By.ID, "first_name").send_keys(
        user_data.get("first_name")
    )
    automation_exercise.find_element(By.ID, "last_name").send_keys(
        user_data.get("last_name")
    )
    automation_exercise.find_element(By.ID, "company").send_keys(
        user_data.get("company")
    )
    automation_exercise.find_element(By.ID, "address1").send_keys(
        user_data.get("address")
    )
    automation_exercise.find_element(By.ID, "country").send_keys(
    ...
    automation_exercise.find_element(By.ID, "mobile_number").send_keys(
        user_data.get("phone")
    )
    automation_exercise.find_element(
        By.CSS_SELECTOR, "[data-qa='create-account']"
    ).click()

    confirmation_message = (
        automation_exercise.find_element(
            By.CSS_SELECTOR, "h2[data-qa='account-created']>b"
        )
    ).text
