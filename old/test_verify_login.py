...
def test_verify_unsuccessful_login(driver):
    ...
    actual_text = wait_until_visible(
        driver, (By.CSS_SELECTOR, ".app-sign-in-register__error")
    ).text
...
def test_verify_dropdown_menu(driver):
    ...
    expected_menu = ["New Item", "Build History", "Manage Jenkins", "My Views"]
    ...
    dropdown_menu = driver.find_element(
        By.CSS_SELECTOR, "#breadcrumbBar .jenkins-menu-dropdown-chevron"
    )
    actions.move_to_element(dashboard_link).move_to_element(
        dropdown_menu
    ).click().perform()

    # Wait for and collect dropdown menu items
    menu_elements = wait_until_visible_all_elements(
        driver, (By.CSS_SELECTOR, "a.jenkins-dropdown__item")
    )

    actual_menu = [item.text.strip() for item in menu_elements]

    assert actual_menu == expected_menu, (
        f"Expected {expected_menu}, but got {actual_menu}"
    )
